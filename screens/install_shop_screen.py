...
class installationScreen():    
    ...
    def install_shop(self,var_test):
        install_ok = False
        import shutil, os
        test = os.listdir(Context().environment.store_source)
        test = sorted(test, reverse=True)
        test2=[]
        for my_list in test:
            if Configuration().version_store in my_list : 
                test2.append(my_list)
        
        test2=sorted(test2,key=len,reverse=True)
        if test2 == []:
            my_version = Configuration().version_store
        else:
            try:
        ...
        if Configuration().git == False:        
            try:
                Context().logger.info("Create folder {0})".format(my_version))
                shutil.copytree(Context().environment.store_source + Configuration().version_store, Context().environment.store_source + my_version)
        ...
        if Configuration().vm == None: 
            url = my_url.replace('str(i)',Configuration().storename).replace('str(j)','localhost')
        else:
            url = my_url.url.replace('str(i)',Configuration().storename).replace('str(j)',Configuration().vm)
    ...
    def reinstall_shop(self,var_test):
        ...
        ui.set_text(self._objects['name'], Configuration().storename,True)
        ui.set_text(self._objects['firstname'], var_test.get("firstname"),True)
        ui.set_text(self._objects['lastname'], var_test.get("lastname"),True)
        ui.set_text(self._objects['email'], var_test.get("email"),True)
        ui.set_text(self._objects['password'], var_test.get("password"),True)
