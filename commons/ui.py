...
def def_object(my_object,value,value2=None):
    ...
    if value2 == None:
        ...
        element = ( my_object[0] + ":|:" + str(my_object[1]).replace("str(i)",str(value)).replace("str(j)",str(value2)))
...
def take_screenshot(filename='screenshot.png'):
    ...
    if config.function_test != None:
        filename = str(config.browser) + "-" + str(config.datasets_file_name).replace("'","").replace("[","").replace("]","") + '-' + str(config.function_test) + '_' + timestamp + '_' + filename
    else:
        filename = str(config.browser) + '-' + str(config.datasets_file_name).replace("'","").replace("[","").replace("]","") + '-' + str(config._fct_test) + '_' + timestamp + '_' + filename
...
def drag_and_drop(source, xoffset, yoffset):
        ...
        actions.append(lambda:Context().browser.execute(Command.MOVE_TO, {'xoffset': int(xoffset),'yoffset': int(yoffset)}))
