...
def keyboard_stream(string_stream):
    mode = False
    for character in string_stream.replace('\r\n', '\r').replace('\n', '\r'):
...
def press_esc():
    ...
    for i in range(n):
        ...
        time.sleep(0.5)
        SendInput(Keyboard(VK_ESCAPE))
        time.sleep(0.2)
        SendInput(Keyboard(VK_ESCAPE, KEYEVENTF_KEYUP))


def press_and_release(key, wait=0.2):
    '''
    Press a given key then wait for an amount of time then release it
    @param key: the key you want to press (as defined in this module)
    @param wait: an amount of secs you want to wait before releasing the key
    '''
